...
| Autoplay(Boolean): Optional  | If set to True, the autoplay functionality displays the items in the Carousel automatically (5 seconds time delay between each item). If set to False, the autoplay functionality is disabled. This is the default value.| 
